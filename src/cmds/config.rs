    ...
    match fs::read_to_string(&path) {
        Ok(content) => {
            ...
            match content.parse::<Value>() {
                Ok(_) => Ok("The file contains valid TOML syntax".to_string()),
                Err(e) => Err(Error::msg(format!("TOML syntax validation failed: {}", e))),
            }
        }
        Err(e) => Err(Error::msg(format!("Failed to read file '{}': {}", path, e))),
    }
}
