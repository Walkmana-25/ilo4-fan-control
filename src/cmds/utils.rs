...
pub fn get_connection_info(host: Option<String>, user: Option<String>, password: Option<String>) -> (String, String, String) {
    ...
    let host: String = host.unwrap_or_else(|| {
        let input: String = Input::new().with_prompt("Enter ilo host").interact_text().unwrap();
        input
    });

    let user: String = user.unwrap_or_else(|| {
        let input: String = Input::new().with_prompt("Enter ilo username").interact_text().unwrap();
        input
    });

    let password: String = password.unwrap_or_else(|| {
        let input: String = Password::new().with_prompt("Enter ilo password").interact().unwrap();
        input
    });
    debug!("Host: {}", host);
    debug!("User: {}", user);
